"use client";
import { createOrdersApi } from "@/services/order.sevice";
...
const CartScreen = () => {
  ...
  const submitOrders = async () => {
	if (formOrders.cart.length === 0) {
		return toast.warning("Giỏ hàng đang trống, vui lòng thêm sản phẩm vào giỏ hàng!", {
		  position: "top-right",
		  autoClose: 5000,
    ...
    try {
      setLoading(true);
      let response = await createOrdersApi(formOrders);
      ...
      return toast.error("Server đang gặp vấn đè, vui lòng thử lại!", {
        position: "top-right",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
